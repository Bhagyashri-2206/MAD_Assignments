...
public class BedTimeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btnNext.setOnClickListener(v -> {
            ...
            Intent intent = new Intent(BedTimeActivity.this, ResultActivity.class);

            intent.putExtra("gender", getIntent().getStringExtra("gender"));
            intent.putExtra("weight", getIntent().getIntExtra("weight", 60));

            intent.putExtra("wakeHour", getIntent().getIntExtra("wakeHour", 6));
            intent.putExtra("wakeMinute", getIntent().getIntExtra("wakeMinute", 0));

            intent.putExtra("sleepHour", pickerHour.getValue());
            intent.putExtra("sleepMinute", pickerMinute.getValue());
